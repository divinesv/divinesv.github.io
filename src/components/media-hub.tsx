...
export function MediaHub({ documents, gitaVerseCount, musicEmbeds, tracks, videos }: MediaHubProps) {
  ...
  const stotraCount = stotraSummaries.length;
  ...
  const discoveryCards: DiscoveryCard[] = [
    ...
    },
    {
      kind: 'link',
      title: 'Stotras',
      body: `${stotraCount} stotra resources spanning full reading pages and library texts.`,
      href: '/stotras',
    ...
    },
    {
      kind: 'tab',
      title: 'Documents',
      body: `${documents.length} library resources collected in one reading space.`,
      tab: 'documents',
    ...
    <section className="section-spacing media-hub-shell">
      ...
      <div className="content-grid card-grid-relaxed media-discovery-grid">
        {discoveryCards.map((card) =>
          card.kind === 'link' ? (
            <Link key={card.title} className="surface-card deity-path-card media-discovery-card" href={card.href}>
              <div className="deity-path-card-visual media-discovery-card-visual">
            ...
            <button
              key={card.title}
              className="surface-card deity-path-card media-discovery-button media-discovery-card"
              type="button"
          ...
          <div className="content-grid card-grid-relaxed">
            {documents.map((pdf) => (
              <article key={pdf.slug} className="surface-card library-document-card">
                ...
                <div className="link-row">
                  <Link className="primary-link" href={pdf.href}>
                    Read Now
                  </Link>
                  {pdf.kind === 'pdf' && pdf.url ? (
